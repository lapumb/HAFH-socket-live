...
The `HAFH-server` uses mTLS to authenticate with any HAFH peripheral. This means that the MQTT broker will only accept connections from clients that have a valid certificate, key, and CA certificate. The `HAFH-server` peripheral key generation is still being worked on, but for now, you _could_ use the example `client.crt`, `client.key`, and `ca.crt` files generated by the [`HAFH-os`](https://github.com/lapumb/HAFH-os) build.

Regardless, you will need to convert the certificates and key to DER format. You can do this using the `openssl` command:
...
**Important note**: I could not find anywhere in the MicroPython documentation that states only DER format is supported. However, I have only been able to get the Pico W to work with DER format certificates.
